...
interference. The Total number of provider threads is two while the total number of buyer
threads is two hundred and sixty.
*/
...
int main(int argc, char **argv)
{
  ...
  //Creates two provider threads
  for(int i = 0; i < 2; i++){
  pthread_create(&t1, NULL, thread_Insert, &thread1);
  }
  
  //Creates two hundred and sixty buyer threads
  for(int i = 0; i < N; i++){
    pthread_create(&t3, NULL, thread_Remove, &thread3);
  }

  // Waiting provider threads to terminate
  for(int i = 0; i < 2; i++){
