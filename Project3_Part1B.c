...
int users_memory[6] = {128, 256, 512, 1024, 2048, 4096}; //User memory sizes
int user_memory_index = 0;
int MAX_SIZE = 2000 * sizeof(int); // 8KB max memory size
...
int main(int argc, char** argv)
{
    ...
    if (state1 != 0){
        puts("Error mutex initialization!!"); // checks mutex to ensure its initialized
        }
...
void* memory_free(int memSize) {
    ...
    if (memSize == 128){
       deallocation = users_memory[0];
    }
    
    else if (memSize == 256){
       deallocation = users_memory[1] + users_memory[0];
    }
    
    else if (memSize == 512){
       deallocation = users_memory[2] + users_memory[1] + users_memory[0];
    }
    
    else if (memSize == 1024){
       deallocation = users_memory[3] + users_memory[2] + users_memory[1] + users_memory[0];
    }
       ...
       deallocation = users_memory[4] + users_memory[3] + users_memory[2] + users_memory[1] + users_memory[0];
    }
